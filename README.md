...
1. Copy the `horcrux/.env.example` to `horcrux/.env` (this will be ignored
   by Git):
...
4. You can edit `horcrux/src/main.leo` and run `leo run` to compile and update the
...
Aleo programs deployed require unique names, make sure to edit the program's name to something unique in `horcrux/src/main.leo`, `horcrux/program.json`, rename `horcrux/inputs/horcrux.in` and rebuild.
